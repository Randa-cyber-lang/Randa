...
local Content = Instance.new("ScrollingFrame") -- Tempat naruh tombol fungsi
...
-- 2. Fungsi Kill Aura (Universal)
local kaEnabled = false
createButton("Toggle Kill Aura", Color3.fromRGB(200, 50, 50), function()
    kaEnabled = not kaEnabled
    print("Kill Aura: " .. tostring(kaEnabled))
    task.spawn(function()
        while kaEnabled do
            local p = game.Players.LocalPlayer
            for _, v in pairs(workspace:GetChildren()) do
                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Name ~= p.Name then
                    local dist = (p.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude
                    if dist < 20 then
                        -- Mencoba panggil Remote Hit secara umum
                        local tool = p.Character:FindFirstChildOfClass("Tool")
                        if tool and tool:FindFirstChild("RemoteClick") then
                            tool.RemoteClick:FireServer(v.Humanoid)
                        end
                    end
                end
            end
